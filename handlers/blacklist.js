...
const truyna = require("../schema/blacklist");
...
module.exports = async (client) => {
  ...
  client.on("ready", async () => {
    ...
    async function updateTruyNa(data, truyna_data) {
      try {
        ...
        for (const x in data) {
          ...
          if (data_old.times >= Math.floor(new Date().getTime() / 1000)) {
            // Luu truy na
     ...
     * @param {String} name
     */
    async function sendNotiDone(data, name) {
      var embed = new MessageEmbed()
        .setTitle("Thong bao het blacklist")
        .setDescription(
          "Ten: " +
            name +
            "\n" +
      ...
      return client.guilds.cache
        .get(config.guild1)
        .channels.cache.get(config.logstruyna)
        .send({ embeds: [embed] });
    }
    ...
    async function sendNoti(data) {
      ...
      if (data.length > 0) {
        for (const x in data) {
          ...
          string += `#${
            Number(x) + 1
          } [ID:${id}] : ${name} blacklist ${time} vi ${reason}\n`;
        }
      } else {
  ...
  client.on("messageCreate", async (msg) => {
    ...
    if (content.startsWith(prefix + "bll")) {
      ...
      var embed;
      var day = 1; //86400s = 1 day
      var blacklist = Math.floor(new Date().getTime() / 1000 + day * time);
